...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  // Define routes where the main navigation should be HIDDEN to allow action buttons to be visible
  const hideNavPaths = [
    '/marketplace/item',
    '/marketplace/post',
    '/inbox/chat',
    '/inbox/group', // includes group-info
  ...
  return (
    <div className="flex flex-col flex-1 h-screen overflow-hidden bg-[#f8f9fa]">
      ...
      {/* If nav is hidden, we don't need the large padding bottom (pb-28), just safe area */}
      <main className={`flex-1 overflow-y-auto custom-scrollbar ${shouldHideNav ? 'pb-safe-bottom' : 'pb-28'}`}>
        {children}
      </main>

      {/* Modern Floating Bottom Nav - Only visible on main tabs */}
      {!shouldHideNav && (
        <div className="fixed bottom-0 left-0 right-0 p-4 safe-bottom z-50 pointer-events-none animate-in slide-in-from-bottom duration-300">
