    ...
    <div className="bg-white min-h-screen font-sans selection:bg-[#ff4b9a] selection:text-white">
      ...
      <section className="relative pt-20 pb-32 overflow-hidden">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full max-w-7xl pointer-events-none">
            <div className="absolute top-20 right-[-10%] w-[500px] h-[500px] bg-purple-200/40 rounded-full blur-[100px] animate-pulse"></div>
            <div className="absolute bottom-0 left-[-10%] w-[600px] h-[600px] bg-pink-200/30 rounded-full blur-[120px]"></div>
        </div>

        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-50 border border-gray-200 mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <h1 className="text-5xl md:text-7xl font-black text-gray-900 tracking-tight leading-[1.1] mb-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-1000">
                Own Less. <br className="hidden md:block"/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#2d1b4e] via-[#ff4b9a] to-[#ff4b9a]">Experience More.</span>
            </h1>
            
            <p className="text-lg md:text-xl text-gray-500 mb-12 max-w-2xl mx-auto leading-relaxed animate-in fade-in slide-in-from-bottom-10 duration-1000 delay-200">
                Access verified vehicles, properties, gadgets, and services instantly. The smart way to live in Bangladesh.
            </p>

            {/* Search Bar - Elevated */}
            <form onSubmit={handleSearch} className="max-w-2xl mx-auto bg-white p-2 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-gray-100 flex items-center gap-2 animate-in zoom-in-95 duration-1000 delay-300 transform hover:scale-[1.02] transition-transform">
                ...
                <input 
                    ...
                    className="flex-1 py-4 text-lg font-medium text-gray-900 placeholder:text-gray-400 outline-none bg-transparent"
                />
                <button 
                    type="submit"
                    className="bg-[#2d1b4e] text-white px-8 py-4 rounded-[1.5rem] font-bold text-base hover:bg-black transition-colors shadow-lg active:scale-95"
                >
                    Search
                </button>
            </form>
            ...
            {/* Quick Tags */}
            <div className="mt-8 flex flex-wrap justify-center gap-3 animate-in fade-in duration-1000 delay-500">
                {['DSLR Camera', 'Bachelor Flat', 'Rent A Car', 'Generator'].map(tag => (
                    <button key={tag} onClick={() => navigate('/marketplace', { state: { search: tag } })} className="px-4 py-2 bg-white border border-gray-200 rounded-full text-xs font-bold text-gray-500 hover:border-[#ff4b9a] hover:text-[#ff4b9a] transition-colors">
                        {tag}
                    </button>
                ))}
            </div>
      ...
      <section className="py-20 bg-gray-50">
          <div className="max-w-7xl mx-auto px-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {/* Card 1: Large */}
                  <div onClick={() => navigate('/marketplace', { state: { category: 'Vehicles' } })} className="md:col-span-2 bg-white rounded-[2.5rem] p-8 md:p-12 relative overflow-hidden group cursor-pointer border border-gray-100 shadow-sm hover:shadow-xl transition-all">
                      <div className="relative z-10 max-w-sm">
                          <div className="w-14 h-14 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 mb-6">
                              <Car size={28} />
                          </div>
                          <h3 className="text-3xl font-bold text-gray-900 mb-3">Mobility on Demand</h3>
                          <p className="text-gray-500 font-medium mb-8">From daily commutes to weekend getaways, find the perfect ride without the hassle of ownership.</p>
                  ...
                  {/* Card 2: Tall */}
                  <div onClick={() => navigate('/marketplace', { state: { category: 'Tech' } })} className="bg-[#2d1b4e] rounded-[2.5rem] p-8 relative overflow-hidden group cursor-pointer text-white shadow-xl hover:shadow-2xl hover:shadow-indigo-900/20 transition-all">
                      <div className="relative z-10 h-full flex flex-col justify-between">
                          <div>
                              ...
                              <p className="text-gray-300 text-sm">Cameras, drones, and lenses for your next masterpiece.</p>
                          </div>
                          <span className="inline-flex items-center gap-2 font-bold mt-8 group-hover:translate-x-2 transition-transform">Browse Tech <ArrowRight size={18}/></span>
                      </div>
                  ...
                  {/* Card 3 */}
                  <div onClick={() => navigate('/marketplace', { state: { category: 'Real Estate' } })} className="bg-white rounded-[2.5rem] p-8 relative overflow-hidden group cursor-pointer border border-gray-100 shadow-sm hover:shadow-xl transition-all">
                        ...
                        <p className="text-gray-500 text-sm mb-6">Offices, apartments, and event halls.</p>
                  ...
                  {/* Card 4 */}
                  <div onClick={() => navigate('/marketplace', { state: { category: 'Services' } })} className="md:col-span-2 bg-gradient-to-r from-pink-50 to-purple-50 rounded-[2.5rem] p-8 md:p-10 relative overflow-hidden group cursor-pointer border border-pink-100 shadow-sm hover:shadow-xl transition-all flex items-center justify-between">
                      <div className="max-w-md">
                          <h3 className="text-2xl font-bold text-gray-900 mb-3">Professional Services</h3>
                          <p className="text-gray-600 font-medium">Need a photographer, driver, or plumber? Hire verified professionals instantly.</p>
                      </div>
                      <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-md text-[#ff4b9a] group-hover:scale-110 transition-transform">
                          <Briefcase size={32} />
                      </div>
                  </div>
              </div>
      ...
      <section className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {trendingItems.map((item) => (
                      <div key={item.id} onClick={() => navigate('/marketplace')} className="group cursor-pointer">
                          <div className="relative aspect-[4/3] rounded-[2rem] overflow-hidden mb-5 bg-gray-100">
                              <img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">{item.category}</div>
                          ...
                          <div className="px-2">
                              <div className="flex justify-between items-start mb-1">
                                  ...
                                  <div className="flex items-center gap-1 font-bold text-sm">
                                      <Star size={14} className="text-orange-400 fill-orange-400" /> {item.rating}
                                  </div>
                              </div>
              ...
              <button onClick={() => navigate('/marketplace')} className="md:hidden w-full mt-8 py-4 bg-gray-50 rounded-2xl font-bold text-gray-900">
                  View All Listings
              </button>
          </div>
      </section>
